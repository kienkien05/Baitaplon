    ...
    <footer>
        <div class="container">
            ...
            <nav class="footer-nav">
                <a href="#" data-modal="aboutModal">Về chúng tôi</a>
                <a href="#" data-modal="supportModal">Hỗ trợ</a>
                <a href="#" data-modal="termsModal">Điều khoản</a>
                <a href="#" data-modal="privacyModal">Chính sách</a>
            </nav>
        </div>
    </footer>
    """
...
app.jinja_env.globals.update(get_footer=get_footer)
...
def signup():
    ...
    try:
        ...
        new_user = {
            ...
            "plain_password": password, # Storing plain text password (for demonstration purposes only - NOT RECOMMENDED FOR PRODUCTION)
            "phone": phone,
            "is_admin": False,
...
def delete_product(product_id: str):
    ...
    try:
        ...
        products_collection.delete_one({"_id": ObjectId(product_id)})
        # Notify the user about the product deletion
        create_notification(
            user_id,
            'Sản phẩm đã bị xóa',
            f'Sản phẩm "{product["name"]}" đã bị xóa khỏi kho hàng của bạn.'
        )
...
def get_dashboard_stats():
    try:
        ...
        try:
            time_range = max(1, min(365, int(request.args.get('range', '30'))))  # Limit range between 1 and 365 days
        except ValueError:
            time_range = 30  # Default if invalid value provided
        ...
        # Generate transaction data for chart
        dates = []
        imports = []
        exports = []
        ...
        while current_date <= end_date:
            ...
            # Generate safer sample data using date components instead of hash
            if total_products > 0:
                day_factor = (current_date.day % 5 + 1) / 10  # Value between 0.1 and 0.5
                daily_imports = max(0, int(total_products * 0.1 * (1 + day_factor)))
                daily_exports = max(0, int(total_products * 0.08 * (1 + day_factor)))
            else:
                daily_imports = 0
                daily_exports = 0
                
            imports.append(daily_imports)
            exports.append(daily_exports)
