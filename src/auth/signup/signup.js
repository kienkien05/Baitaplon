class SignupManager {
    ...
    async handleSignup(e) {
        ...
        for (let error of errorElements) {
            if (error.textContent) {
                showTemporaryNotification('Vui lòng sửa các lỗi trước khi đăng ký', 'error');
                return;
            }
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                showTemporaryNotification(data.error || 'Có lỗi xảy ra khi đăng ký!', 'error');
            }
        } catch (error) {
            console.error('Signup error:', error);
            showTemporaryNotification('Lỗi kết nối, vui lòng thử lại sau!', 'error');
        }
    }
    ...
    validateForm(data) {
        ...
        if (!this.validateUsername(data.username)) {
            showTemporaryNotification('Tên đăng nhập không hợp lệ', 'error');
            return false;
        }

        if (!this.validateEmail(data.email)) {
            showTemporaryNotification('Email không hợp lệ', 'error');
            return false;
        }
        ...
        if (data.password !== data.confirmPassword) {
            showTemporaryNotification('Mật khẩu xác nhận không khớp', 'error');
            return false;
        }
