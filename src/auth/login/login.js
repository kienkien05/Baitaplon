class LoginForm {
    ...
    setup() {
        console.log('LoginForm setup function called'); // Log A
        this.form = document.getElementById('loginForm');
        console.log('LoginForm form element:', this.form); // Log B

        if (this.form) {
            this.form.addEventListener('submit', this.handleLogin.bind(this));
            console.log('Login form submit event listener attached'); // Log C
        } else {
            console.error('Login form element not found!'); // Log D
        }
    ...
    async handleLogin(e) {
        console.log('handleLogin function called'); // Log 0
        e.preventDefault();
        const username = document.getElementById('username').value.trim();
        const password = document.getElementById('password').value;
        const errorMessage = document.getElementById('errorMessage');

        console.log('Attempting login for username:', username); // Log 1

        if (!username || !password) {
            showTemporaryNotification('Vui lòng điền đầy đủ thông tin!', 'error');
            console.log('Missing username or password.'); // Log 2
            return;
        }

        try {
            console.log('Sending login request to /auth/login'); // Log 3
            ...
            console.log('Received response:', response.status); // Log 4

            const data = await response.json();
            console.log('Parsed response data:', data); // Log 5
            
            if (data.success) {
                console.log('Login successful, redirecting to:', data.redirect); // Log 6
                window.location.href = data.redirect;
            } else {
                console.log('Login failed:', data.error); // Log 7
                showTemporaryNotification(data.error, 'error');
            }
        } catch (error) {
            console.error('Login fetch error:', error); // Log 8
            showTemporaryNotification('Có lỗi xảy ra khi đăng nhập!', 'error');
        }
    }
    ...
    async handleForgotPassword() {
        ...
        if (!email) {
            showTemporaryNotification('Vui lòng nhập địa chỉ email!', 'error');
            return;
        }
        ...
        try {
            ...
            showTemporaryNotification('Đã gửi yêu cầu khôi phục mật khẩu! Vui lòng kiểm tra email.', 'success');
            this.hideForgotPasswordForm();
        } catch (err) {
            showTemporaryNotification('Có lỗi khi gửi yêu cầu khôi phục mật khẩu!', 'error');
            console.error(err);
        }
    }
