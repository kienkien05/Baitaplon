...
async function fetchNotifications() {
    try {
        ...
        if (notifications.length === 0) {
            notificationList.innerHTML = '<p class="no-notifications">Không có thông báo nào</p>';
...
notificationBtn.addEventListener('click', fetchNotifications);
markAllReadBtn.addEventListener('click', markAllNotificationsRead);

// Initial fetch
fetchNotifications();
// Real-time product existence check
const productNameInput = document.getElementById('productName');
const productExistsMessage = document.getElementById('productExistsMessage');

if (productNameInput && productExistsMessage) {
    productNameInput.addEventListener('input', async () => {
        const productName = productNameInput.value.trim();
        if (!productName) {
            productExistsMessage.textContent = '';
            return;
        }
        ...
        try {
            ...
            if (data.exists) {
                productExistsMessage.textContent = 'Sản phẩm đã tồn tại.';
                productExistsMessage.style.color = 'red';
            } else {
                productExistsMessage.textContent = 'Sản phẩm chưa tồn tại.';
                productExistsMessage.style.color = 'green';
            }
        } catch (error) {
            console.error('Error checking product existence:', error);
            productExistsMessage.textContent = 'Không thể kiểm tra sản phẩm.';
            productExistsMessage.style.color = 'orange';
        }
    });
}
    }
